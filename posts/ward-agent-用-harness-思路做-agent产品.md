...
如果不加干预，AI 不会知道用户正在看哪个指数——它只能靠 prompt 里模糊的指令自己判断要查什么，可能调错工具，可能重复查询浪费 API。

**Harness 的思路**：与其让 AI 靠 prompt "猜"当前状态，不如把系统状态直接喂给它。工具再多、环境再复杂，只要 AI 能清晰感知"现在是什么状态"，它自然知道该用什么。
...
Ward 没有用 RAG 或向量数据库，而是用**自动摘要**：当 token 数量超过阈值（8万），把历史压缩成一段可读的摘要，替换掉中间的执行过程。
