...
如果不加干预，AI 会调工具重新查一遍标普500的数据——浪费 API 调用，响应更慢。更麻烦的是，AI 可能查到不同的数据源，导致和页面展示的数字不一致。

**Harness 的思路**：与其让 AI 靠 prompt "猜"当前状态，不如把系统状态直接喂给它。
...
用户可能在 AI 执行过程中改变主意。Ward 支持按 Esc 取消。
...
**Harness 的思路**：把长期状态外化到外部存储，按需读取。

Ward 的做法是**自动摘要**：当 token 数量超过 8 万阈值，用 LLM 把历史压缩成一段摘要。
...
策略是：**用户说的话不压缩，AI 的执行过程压缩**。用户意图是上下文的核心，不能损失；AI 做了什么（调了哪些工具、返回了什么中间结果）相对次要，压缩掉没问题。

```python
# 压缩内容：保留用户原始提问，AI 执行过程变成一段话
