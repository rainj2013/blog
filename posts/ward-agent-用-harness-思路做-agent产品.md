...
Ward 的做法是：每次对话开始，把页面当前数据以结构化文本注入 system prompt，同时把工具列表和用途一并附上。AI 看到"上下文里有 spx 的60日K线"，就知道不需要调工具，直接分析即可；如果上下文没有某只股票的数据，才会去调 `get_stock_kline` 等工具获取。上下文有什么、缺什么、该调什么——全由 system prompt 交代清楚，不需要用户额外说明"我当前在看什么"。
